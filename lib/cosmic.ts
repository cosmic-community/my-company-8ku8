...
import { Division, Service, TeamMember, CaseStudy, Testimonial, hasStatus } from '@/types'
...
export async function getDivisions(): Promise<Division[]> {
  try {
    const response = await cosmic.objects
      ...
      .depth(1)
    const divisions = response.objects as Division[]
...
export async function getDivision(slug: string): Promise<Division | null> {
  try {
    const response = await cosmic.objects
      .findOne({ type: 'divisions', slug })
      .depth(1)
    return response.object as Division
  } catch (error) {
